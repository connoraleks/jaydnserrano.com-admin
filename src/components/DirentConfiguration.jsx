...
const DirentConfiguration = ({ setTriggerRefresh, newDirent, currentDirent, openModal, setOpenModal, sections}) => {
    ...
    const [name, setName] = useState(sections ? currentDirent.name.slice(0, currentDirent.name.lastIndexOf('.')) : '');
    ...
    const [fileExtension, ] = useState(type === 0 ? currentDirent.name.slice(currentDirent.name.lastIndexOf('.') + 1 ) : '');
    ...
    const onUpload = () => {
        ...
        // If directory, upload directory to api.jaydnserrano.com/dirents
            ...
            formData.append('type', type);
            formData.append('parent', currentDirent.id);
            console.log(name, type, currentDirent);
            axios.post('https://api.jaydnserrano.com/dirents', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
    ...
    const onEdit = () => {
        const formData = new FormData();
        formData.append('name', name + (type === 0 ? '.' + fileExtension : ''));
        formData.append('parent', sections[Object.keys(sections)[parent]] ? sections[Object.keys(sections)[parent]] : -1);
